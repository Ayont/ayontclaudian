...
/**
 * Filesystem layout helpers for the Grok (`grok-cli`) data directory:
 *
 *   ~/.grok/
 ...
 * directory is not contractually fixed by the CLI, so `readGrokSessionLog`
 * resolves the newest NDJSON-looking file in the directory defensively.
 */

const GROK_DATA_SUBDIR = '.grok';
const SESSIONS_SUBDIR = 'sessions';
const CONFIG_FILENAME = 'config.toml';
const SESSION_LOG_EXTENSIONS = ['.jsonl', '.ndjson', '.json'];

/** Root data directory for `grok-cli` (honors `GROK_HOME` if set). */
export function getGrokDataDir(): string {
  const override = process.env.GROK_HOME?.trim();
  if (override) {
    return override;
  }
  return path.join(os.homedir(), GROK_DATA_SUBDIR);
}

/** Absolute path to `~/.grok/config.toml`. */
export function getGrokConfigPath(): string {
  return path.join(getGrokDataDir(), CONFIG_FILENAME);
}

/** The `sessions/` directory that contains one subdirectory per session. */
export function getGrokSessionsDir(): string {
  return path.join(getGrokDataDir(), SESSIONS_SUBDIR);
}

/** Absolute session directory for a single session id. */
export function getGrokSessionDir(sessionId: string): string {
  return path.join(getGrokSessionsDir(), sessionId);
}
 ...
 */
export function getGrokSessionFilePath(sessionId: string): string | null {
  const dir = getGrokSessionDir(sessionId);
  let names: string[];
  try {
...
export function readGrokSessionLog(sessionId: string): string | null {
  const file = getGrokSessionFilePath(sessionId);
  if (!file) {
    return null;
  }
  try {
...
export function listGrokSessionIds(): string[] {
  const dir = getGrokSessionsDir();
  let names: string[];
  try {
    names = fs.readdirSync(dir);
  } catch {
...
export function deleteGrokSessionDir(sessionId: string): void {
  const dir = getGrokSessionDir(sessionId);
  try {
