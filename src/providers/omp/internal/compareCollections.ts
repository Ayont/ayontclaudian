import type { OmpDiscoveredModel, OmpThinkingOptionsByModel } from '../models';
import type { OmpMode } from '../modes';
...
export function sameModes(left: OmpMode[], right: OmpMode[]): boolean {
  if (left.length !== right.length) {
    return false;
  }
...
export function sameThinkingOptionsByModel(
  left: OmpThinkingOptionsByModel,
  right: OmpThinkingOptionsByModel,
): boolean {
  const leftEntries = Object.entries(left);
  if (leftEntries.length !== Object.keys(right).length) {
    return false;
