...
export function createOmpAgentPersistenceKey(
  location: OmpAgentLocation,
): string {
  return `${OMP_AGENT_PERSISTENCE_PREFIX}:${encodeURIComponent(normalizeVaultPath(location.filePath))}`;
}

export function parseOmpAgentPersistenceKey(
  persistenceKey?: string,
): OmpAgentLocation | null {
  if (!persistenceKey) {
    return null;
  }
  ...
  if (prefix !== OMP_AGENT_PERSISTENCE_PREFIX || !encodedRelativePath) {
    return null;
  }
  ...
  return decoded.endsWith('.md')
    ? { filePath: `${OMP_AGENTS_PATH}/${decoded}` }
    : null;
}

export class OmpAgentStorage {
  constructor(
    private vaultAdapter: Pick<VaultFileAdapter, 'exists' | 'read' | 'write' | 'delete' | 'listFilesRecursive' | 'ensureFolder'>,
  ) {}

  async loadAll(): Promise<OmpAgentDefinition[]> {
    return this.scanAdapter(this.vaultAdapter);
  }

  async load(agent: OmpAgentDefinition): Promise<OmpAgentDefinition | null> {
    ...
    try {
      ...
      return parseOmpAgentMarkdown(content, filePath);
    } catch {
      return null;
    }
  }

  async save(agent: OmpAgentDefinition, previous?: OmpAgentDefinition | null): Promise<void> {
    ...
    const content = serializeOmpAgentMarkdown(agent);
  ...
  async delete(agent: OmpAgentDefinition): Promise<void> {
    const filePath = this.resolveCurrentPath(agent);
    await this.vaultAdapter.delete(filePath);
  }

  private resolveCurrentPath(agent: OmpAgentDefinition): string {
    const persistedLocation = parseOmpAgentPersistenceKey(agent.persistenceKey);
    if (persistedLocation) {
      return persistedLocation.filePath;
    }

    return `${OMP_DEFAULT_AGENT_SAVE_PATH}/${agent.name}.md`;
  }

  private resolveTargetPath(
    agent: OmpAgentDefinition,
    previous?: OmpAgentDefinition | null,
  ): string {
  ...
  ): Promise<OmpAgentDefinition[]> {
    const agentsByName = new Map<string, OmpAgentDefinition>();

    for (const rootPath of OMP_AGENT_SCAN_PATHS) {
      try {
        ...
        for (const filePath of files) {
          ...
          try {
            ...
            const agent = parseOmpAgentMarkdown(content, filePath);
            if (!agent) continue;
...
export function parseOmpAgentMarkdown(
  content: string,
  filePath: string,
): OmpAgentDefinition | null {
  ...
  const result: OmpAgentDefinition = {
    name,
    description,
    prompt: parsed.body.trim(),
    persistenceKey: createOmpAgentPersistenceKey({
  ...
  for (const [key, value] of Object.entries(frontmatter)) {
    if (!OMP_AGENT_KNOWN_KEYS.has(key)) {
...
export function serializeOmpAgentMarkdown(agent: OmpAgentDefinition): string {
  const lines: string[] = ['---'];
...
function normalizeMode(value: unknown): OmpAgentDefinition['mode'] | undefined {
...
function isSupportedAgentFilePath(filePath: string): boolean {
  return OMP_AGENT_SCAN_PATHS.some((rootPath) => filePath.startsWith(`${rootPath}/`))
