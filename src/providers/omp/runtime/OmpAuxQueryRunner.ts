...
import { OMP_PLAN_MODE_ID } from '../modes';
...
type OmpAuxArtifactPurpose = 'inline' | 'instructions' | 'title-gen';

interface OmpAuxQueryRunnerOptions {
  artifactPurpose: OmpAuxArtifactPurpose;
  allowReadTextFile?: boolean;
}

export class OmpAuxQueryRunner implements AuxQueryRunner {
  ...
  constructor(
    ...
    private readonly options: OmpAuxQueryRunnerOptions,
  ) {}
  ...
  async query(config: AuxQueryConfig, prompt: string): Promise<string> {
    ...
    if (!this.connection) {
      throw new Error('OMP runtime is not ready.');
    }

    if (!this.sessionId) {
      const sessionId = await this.createSession(cwd);
      if (!sessionId) {
        throw new Error('Failed to create an OMP session.');
      }
    }
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : 'OMP request failed';
  ...
  private async ensureReady(cwd: string, systemPrompt: string): Promise<void> {
    ...
    const runtimeEnv = buildOmpRuntimeEnv(settings, resolvedCliPath);
    ...
    const nextLaunchKey = JSON.stringify({
      ...
      envText: getRuntimeEnvironmentText(settings, 'omp'),
      purpose: this.options.artifactPurpose,
    });
  ...
  private async createSession(cwd: string): Promise<string | null> {
    ...
    try {
      ...
      // Oh My Pi's only read-only posture is its `plan` mode; hidden
      // title/refine/verifier calls must never be able to write to the vault.
      await this.connection.setConfigOption({
        configId: 'mode',
        sessionId: response.sessionId,
        type: 'select',
        value: OMP_PLAN_MODE_ID,
      });
  ...
  private resolveSelectedRawModel(explicitModel?: string): string | undefined {
    ...
    if (explicitModel) {
      ...
      return ompChatUIConfig.ownsModel(trimmed, projectedSettings)
        ? decodeOmpModelId(trimmed) ?? undefined
        : trimmed;
    }
    ...
    const selectedModel = typeof projectedSettings.model === 'string'
      ...
      : '';
    return ompChatUIConfig.ownsModel(selectedModel, projectedSettings)
      ? decodeOmpModelId(selectedModel) ?? undefined
      : undefined;
  }
  ...
  private resolveSessionPath(sessionId: string, rawPath: string): string {
    ...
    throw new Error('OMP aux read access is limited to the current workspace.');
  }
}
