...
import { buildOmpPromptBlocks, buildOmpPromptText } from './buildOmpPrompt';
import { prepareOmpLaunchArtifacts } from './OmpLaunchArtifacts';
import { buildOmpRuntimeEnv } from './OmpRuntimeEnvironment';
...
export class OmpChatRuntime implements ChatRuntime {
  ...
  getCapabilities(): Readonly<ProviderCapabilities> {
    return OMP_PROVIDER_CAPABILITIES;
  }
  ...
  prepareTurn(request: ChatTurnRequest): PreparedChatTurn {
    return {
      ...
      persistedContent: '',
      prompt: buildOmpPromptText(request),
      request,
    };
  }
  ...
  async warmModelMetadata(model: string): Promise<boolean> {
    const selectedRawModelId = decodeOmpModelId(model);
    ...
    const discoveredModels = getOmpProviderSettings(this.plugin.settings).discoveredModels;
    const selectedBaseRawModelId = resolveOmpBaseModelRawId(selectedRawModelId, discoveredModels);
  ...
  async ensureReady(options?: ChatRuntimeEnsureReadyOptions): Promise<boolean> {
    ...
    const resolvedCliPath = this.plugin.getResolvedProviderCliPath('omp') ?? 'omp';
    const runtimeEnv = this.buildRuntimeEnv(resolvedCliPath);
    const promptSettings = this.getSystemPromptSettings(cwd);
    const artifacts = prepareOmpLaunchArtifacts({ settings: promptSettings });

    const nextLaunchKey = JSON.stringify({
      command: resolvedCliPath,
      envText: getRuntimeEnvironmentText(this.plugin.settings, 'omp'),
    ...
    if (shouldRestart) {
      ...
      await this.startProcess({
        ...
        cwd,
        runtimeEnv,
        systemPromptText: artifacts.systemPromptText,
      });
      this.currentLaunchKey = nextLaunchKey;
  ...
  async *query(
    ...
    if (!(await this.ensureReady())) {
      yield { type: 'error', content: 'Failed to start OMP. Check the CLI path and login state.' };
      yield { type: 'done' };
      return;
    }

    if (!this.connection) {
      yield { type: 'error', content: 'OMP runtime is not ready.' };
      yield { type: 'done' };
      return;
    }
    ...
    if (!this.sessionId) {
      ...
      if (!sessionId) {
        yield { type: 'error', content: 'Failed to create an OMP session.' };
        yield { type: 'done' };
        return;
      }
    }
    ...
    const activeTurn = this.activeTurn;
      ...
      console.warn("Failed to apply OMP mode:", error);
    }
    try {
      await this.applySelectedModel(sessionId, queryOptions);
    } catch (error) {
      console.warn("Failed to apply OMP model:", error);
    }
    try {
      await this.applySelectedEffort(sessionId);
    } catch (error) {
      console.warn("Failed to apply OMP effort:", error);
    }

    const promptPromise = this.connection.prompt({
      prompt: buildOmpPromptBlocks(
        turn.request,
      ...
      for (const chunk of finishAcpTurnChunks({
        compacted: isAcpCompactPrompt(turn.request.text, OMP_PROVIDER_CAPABILITIES.compact?.command),
        usage,
        sessionId,
      })) {
  ...
  buildSessionUpdates(params: {
    ...
    const providerState: OmpProviderState = {};
  ...
  private async startProcess(params: {
    ...
    // omp accepts its global flags after the subcommand, and the prompt goes in
    // as literal text (see OmpLaunchArtifacts for why it is not a path).
    const args = ['acp', `--cwd=${params.cwd}`];
    if (params.systemPromptText.trim()) {
      args.push('--append-system-prompt', params.systemPromptText);
    }
  ...
  private buildRuntimeEnv(cliPath: string): NodeJS.ProcessEnv {
    return buildOmpRuntimeEnv(this.plugin.settings, cliPath);
  }
  ...
  private resolveSelectedRawModelId(queryOptions?: ChatRuntimeQueryOptions): string | null {
    ...
    if (!isOmpModelSelectionId(selectedModel)) {
      return null;
    }

    const selectedBaseRawModelId = decodeOmpModelId(selectedModel);
    if (!selectedBaseRawModelId) {
      return null;
    }

    const discoveredModels = getOmpProviderSettings(providerSettings).discoveredModels;
    const normalizedBaseRawModelId = resolveOmpBaseModelRawId(selectedBaseRawModelId, discoveredModels);
  ...
  private getActiveDisplayModel(queryOptions?: ChatRuntimeQueryOptions): string | undefined {
    ...
    if (
      selectedModel
      && selectedModel !== OMP_SYNTHETIC_MODEL_ID
      && isOmpModelSelectionId(selectedModel)
    ) {
      const selectedRawModelId = this.resolveSelectedRawModelId(queryOptions);
      return selectedRawModelId
        ? encodeOmpModelId(selectedRawModelId)
        : selectedModel;
    }

    return this.currentSessionModelId
      ? encodeOmpModelId(this.currentSessionModelId)
      : (selectedModel && isOmpModelSelectionId(selectedModel) ? selectedModel : undefined);
  }

  private resolveSelectedModeId(): string | null {
    const providerSettings = this.getProviderSettings();
    const ompSettings = getOmpProviderSettings(providerSettings);
    const availableModes = getManagedOmpModes(ompSettings.availableModes);
    const mappedModeId = resolveOmpModeForPermissionMode(
      providerSettings.permissionMode,
      ompSettings.availableModes,
    );
    if (mappedModeId) {
      return mappedModeId;
    }

    if (ompSettings.selectedMode) {
      if (
        availableModes.some((mode) => mode.id === ompSettings.selectedMode)
      ) {
        return ompSettings.selectedMode;
      }
    }
  ...
  private resolveSelectedEffortValue(): string | null {
    ...
    const selectedEffort = typeof providerSettings.effortLevel === 'string'
      ...
      : '';
    if (!selectedEffort || selectedEffort === OMP_DEFAULT_THINKING_LEVEL) {
      return null;
    }
  ...
  private async syncSessionModelState(params: {
    ...
    const currentSettings = getOmpProviderSettings(settingsBag);
    const currentBaseRawModelId = currentRawModelId
      ? resolveOmpBaseModelRawId(currentRawModelId, discoveredModels)
      : null;
    const thoughtLevelState = extractAcpSessionThoughtLevelState(params);
    const currentThinkingOptions = normalizeOmpModelVariants(
  ...
  private seedActiveModelSelection(
    ...
    const savedModel = typeof savedProviderModel.omp === 'string'
      ? savedProviderModel.omp
      : '';
    if (!savedModel || savedModel === OMP_SYNTHETIC_MODEL_ID) {
      savedProviderModel.omp = modelSelection;
      changed = true;
    }

    if (thinkingLevel) {
      const savedProviderEffort = ensureProviderProjectionMap(settingsBag, 'savedProviderEffort');
      const savedEffort = typeof savedProviderEffort.omp === 'string'
        ? savedProviderEffort.omp.trim()
        : '';
      if (!savedEffort || savedEffort === OMP_DEFAULT_THINKING_LEVEL) {
        savedProviderEffort.omp = thinkingLevel;
        changed = true;
      }
    }
  ...
  private async syncSessionModeState(params: {
    ...
    const availableModes = normalizeOmpAvailableModes(acpState.availableModes);
    ...
    if (shouldSeedSelectedMode && currentModeId) {
      updateOmpProviderSettings(settingsBag, { selectedMode: currentModeId });
      await this.plugin.saveSettings();
    }
    this.refreshModelSelectors();
  }
  ...
  private emitPermissionModeSync(modeId: string): void {
    const permissionMode = resolvePermissionModeForManagedOmpMode(modeId);
  ...
  private async handlePermissionRequest(
    ...
    const presentation = buildOmpPermissionPresentation(request.toolCall.title, input, request.toolCall.locations);
  ...
  private formatRuntimeError(error: unknown): string {
    const baseMessage = error instanceof Error ? error.message : 'OMP request failed';
...
function buildOmpPermissionPresentation(
  ...
  switch (permissionId) {
    case 'bash':
      return {
        ...
        description: 'OMP wants to run a shell command.',
        toolName: 'bash',
      };
    case 'codesearch':
      return {
        description: 'OMP wants to search indexed code outside the active buffer.',
        toolName: 'codesearch',
      };
    case 'doom_loop': {
      const repeatedTool = typeof input.tool === 'string' ? input.tool.trim() : '';
      return {
        decisionReason: 'OMP detected repeated identical tool calls',
    ...
    case 'edit':
      return {
        ...
        description: blockedPath
          ? 'OMP wants to modify this file.'
          : 'OMP wants to apply file changes.',
        toolName: 'edit',
      };
    case 'external_directory':
      return {
        ...
        description: blockedPath
          ? 'OMP wants to access a path outside the working directory.'
          : 'OMP wants to access files outside the working directory.',
        toolName: 'External Directory',
      };
    case 'glob':
      return {
        description: 'OMP wants to scan file paths with a glob pattern.',
        toolName: 'glob',
      };
    case 'grep':
      return {
        description: 'OMP wants to search file contents with a pattern.',
        toolName: 'grep',
      };
    case 'lsp':
      return {
        description: 'OMP wants to query language server data.',
        toolName: 'lsp',
      };
    case 'plan_enter':
      return {
        description: 'OMP wants to switch this session into planning mode.',
        toolName: 'Enter Plan Mode',
      };
    case 'plan_exit':
      return {
        description: 'OMP wants to leave planning mode and resume implementation.',
        toolName: 'Exit Plan Mode',
      };
    case 'question':
      return {
        description: 'OMP wants to ask you a direct question before continuing.',
    ...
    case 'read':
      return {
        ...(blockedPath ? { blockedPath } : {}),
        description: blockedPath
          ? 'OMP wants to read this path.'
          : 'OMP wants to read project files.',
        toolName: 'read',
      };
    case 'skill':
      return {
        description: 'OMP wants to load a skill into the current session.',
        toolName: 'skill',
      };
    case 'todowrite':
      return {
        description: 'OMP wants to update the shared task list.',
        toolName: 'todowrite',
      };
    case 'webfetch':
      return {
        description: 'OMP wants to fetch content from a URL.',
        toolName: 'webfetch',
      };
    case 'websearch':
      return {
        description: 'OMP wants to search the web.',
        toolName: 'websearch',
      };
    ...
    default:
      return {
        ...(blockedPath ? { blockedPath } : {}),
        description: blockedPath
          ? `OMP wants permission to use ${formatPermissionLabel(permissionId)} on this path.`
          : `OMP wants permission to use ${formatPermissionLabel(permissionId)}.`,
