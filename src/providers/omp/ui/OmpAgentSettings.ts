...
const OMP_AGENT_INVALID_SEGMENT_PATTERN = /[<>:"\\|?*]/;

export function validateOmpAgentName(name: string): string | null {
  ...
  for (const segment of segments) {
    ...
    if (segment.includes('\0') || OMP_AGENT_INVALID_SEGMENT_PATTERN.test(segment)) {
...
export function findOmpAgentNameConflict(
  agents: OmpAgentDefinition[],
  name: string,
...
class OmpAgentModal extends Modal {
  private existing: OmpAgentDefinition | null;
  private allAgents: OmpAgentDefinition[];
  private onSave: (agent: OmpAgentDefinition) => Promise<void>;

  constructor(
    app: App,
    existing: OmpAgentDefinition | null,
    allAgents: OmpAgentDefinition[],
    onSave: (agent: OmpAgentDefinition) => Promise<void>,
  ) {
    super(app);
  ...
  onOpen() {
    this.setTitle(this.existing ? 'Edit OMP Subagent' : 'Add OMP Subagent');
    ...
    new Setting(contentEl)
      .setName('Name')
      .setDesc('OMP-Agentenname. Verschachtelte Agenten werden mit Schrägstrichen getrennt.')
      .addText((text) => {
    ...
    new Setting(contentEl)
      ...
      .setDesc('Wann OMP diesen Subagenten einsetzen soll')
      .addText((text) => {
    ...
    saveBtn.addEventListener('click', () => {
      ...
      const nameError = validateOmpAgentName(name);
      if (nameError) {
      ...
      const duplicate = findOmpAgentNameConflict(
        this.allAgents,
        name,
      ...
      const agent: OmpAgentDefinition = {
        name,
        description,
        prompt,
        mode: 'subagent',
...
export class OmpAgentSettings {
  private containerEl: HTMLElement;
  private storage: OmpAgentStorage;
  ...
  async render(): Promise<void> {
    ...
    headerEl.createSpan({ text: 'OMP Subagents', cls: 'claudian-sp-label' });
    ...
    if (visibleAgents.length === 0) {
      ...
      emptyEl.setText('No OMP subagents in vault. Click + to create one.');
      return;
    }
  ...
  private renderItem(listEl: HTMLElement, agent: OmpAgentDefinition): void {
  ...
  private openModal(existing: OmpAgentDefinition | null): void {
    if (!this.app) return;

    const modal = new OmpAgentModal(
      this.app,
      existing,
      this.agents,
      async (agent) => {
